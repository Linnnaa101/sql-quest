...
const standardTimeChallenge = logic.selectTimeChallengeLevels(LEVELS, solved, () => true, () => 0.5);
assert.equal(standardTimeChallenge.length, 5, 'Eine neue Zeit-Challenge enthält standardmäßig 5 Level.');
assert.equal(new Set(standardTimeChallenge.map(level => level.id)).size, standardTimeChallenge.length, 'Zeit-Challenge-Level enthalten keine Duplikate.');
assert.equal(standardTimeChallenge.every(level => logic.isLevelUnlocked(LEVELS, solved, indexOf(level.id), { isTestMode: true, areAllLevelsUnlockedForTesting: true })), true, 'Alle Challenge-Level sind freigeschaltet.');
const firstRandomChallenge = logic.selectTimeChallengeLevels(LEVELS, solved, () => true, () => 0.1).map(level => level.id).join(',');
const secondRandomChallenge = logic.selectTimeChallengeLevels(LEVELS, solved, () => true, () => 0.9).map(level => level.id).join(',');
assert.notEqual(firstRandomChallenge, secondRandomChallenge, 'Neuer Start erzeugt eine neue Auswahl.');
const scarceChallenge = logic.selectTimeChallengeLevels(LEVELS, emptyProgress(), (_level, index) => index === 0);
assert.deepEqual(scarceChallenge.map(level => level.id), [1], 'Bei sehr wenigen verfügbaren Leveln startet die Challenge sauber mit weniger als 5 Leveln.');
const preferredUnsolvedLevels = logic.selectTimeChallengeLevels(LEVELS, unlockedTimeChallengeProgress, unlockedForProgress(unlockedTimeChallengeProgress), () => 0);
assert.equal(preferredUnsolvedLevels[0].id, 2, 'Ungelöste Level werden in der Mehrlevel-Challenge bevorzugt.');
const preferredUnderThreeLevels = logic.selectTimeChallengeLevels(LEVELS, { solvedLevelIds: [1, 2], levelStars: { 1: 3, 2: 2 } }, (_level, index) => index < 2, () => 0);
assert.equal(preferredUnderThreeLevels[0].id, 2, 'Level mit weniger als 3 Sternen werden danach bevorzugt.');
...
assert.deepEqual(logic.normalizeTimeChallenge({ solvedLevelIds: [], levelStars: {} }).timeChallenge, { bestRemainingSecondsByLevel: {}, completedCount: 0, completedChallengeCount: 0, bestRemainingSeconds: 0, bestExpiredSolvedCount: 0, lastStartedLevelId: null }, 'Reset entfernt gespeicherte Zeit-Challenge-Daten.');
assert.deepEqual(logic.normalizeTimeChallenge({ solvedLevelIds: [1], levelStars: { 1: 3 } }).timeChallenge, { bestRemainingSecondsByLevel: {}, completedCount: 0, completedChallengeCount: 0, bestRemainingSeconds: 0, bestExpiredSolvedCount: 0, lastStartedLevelId: null }, 'Alte Fortschrittsdaten ohne Zeit-Challenge-Felder funktionieren.');
const testModeTimeLevels = logic.selectTimeChallengeLevels(LEVELS, solved, () => true, () => 0.4);
assert.equal(testModeTimeLevels.length, 5, 'Testmodus bleibt für Zeit-Challenges kompatibel.');
let completedChallengeProgress = logic.recordCompletedTimeChallenge({ solvedLevelIds: [], levelStars: {} }, 99);
completedChallengeProgress = logic.recordCompletedTimeChallenge(completedChallengeProgress, 80);
assert.equal(completedChallengeProgress.timeChallenge.completedChallengeCount, 2, 'Vollständig abgeschlossene Zeit-Challenges werden gezählt.');
assert.equal(completedChallengeProgress.timeChallenge.bestRemainingSeconds, 99, 'Beste Restzeit einer vollständig abgeschlossenen Challenge wird gespeichert.');
const expiredChallengeProgress = logic.recordExpiredTimeChallenge(completedChallengeProgress, 3);
assert.equal(expiredChallengeProgress.timeChallenge.bestExpiredSolvedCount, 3, 'Beste Anzahl gelöster Challenge-Level bei Zeitablauf wird gespeichert.');
assert.equal(/completeActiveTimeChallenge\(level\.id\)/.test(appCode), true, 'Nach korrekter Lösung wird die aktive Challenge fortgeschaltet.');
assert.equal(/finishTimeChallenge\(true\)/.test(appCode), true, 'Nach dem letzten Challenge-Level endet die Challenge.');
assert.equal(/finishTimeChallenge\(false\)/.test(appCode), true, 'Bei Zeitablauf endet die Challenge.');
